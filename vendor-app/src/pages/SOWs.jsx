...
export default function ClientSOWs() {
  ...
  const reject = async () => {
    ...
    setError('');
    try {
      const res = await api.post(`/sows/${selected.id}/reject`, { reason: rejectReason });
      setSelected(res.sow);
      setRejectMode(false);
      setRejectReason('');
      load();
  ...
  return (
    <>
      <PageHeader
        icon={ScrollText}
        title="SOWs & Quotes"
        subtitle={isFinance ? 'Approve SOWs from the delivery team to advance them to DocuSign' : isManager ? 'Sign approved SOWs to activate engagements' : 'Track all Statements of Work for your account'}
      />
          ...
          <div className="space-y-4">
            ...
            <div className="border border-slate-200 rounded-xl divide-y divide-slate-100">
              <div className="flex items-center gap-3 p-3">
                ...
                <span className="text-[12px] text-slate-500 flex-1">Created by delivery team</span>
            ...
            {rejectMode ? (
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                ...
                <div className="flex items-center gap-2">
                  {/* Finance can approve when status is submitted_for_finance */}
                  {isFinance && selected.status === 'submitted_for_finance' && (
                    <>
