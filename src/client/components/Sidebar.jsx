...
import {
  ...
  ScrollText, Users, Building2, Receipt, FileCheck, Grid3x3, FileSignature,
} from 'lucide-react';
...
const navSections = [
  ...
  },
  {
    title: 'Approvals',
    items: [
      { to: '/client/sows', icon: ScrollText, label: 'SOWs & Quotes' },
      { to: '/client/sow-manager', icon: FileSignature, label: 'SOW Manager', only: 'finance' },
      { to: '/client/timesheets', icon: FileCheck, label: 'Timesheets' },
  ...
  },
  {
    title: 'Engagement',
    items: [
      { to: '/client/roster', icon: Users, label: 'Project Roster' },
      { to: '/client/nine-box', icon: Grid3x3, label: '9-Box Grid' },
      { to: '/client/vendors', icon: Building2, label: 'Vendors' },
    ],
  },
];
...
export default function Sidebar() {
  ...
  // Menus differ by subrole — e.g. SOW Manager is finance-only. Filter out any
  // item whose `only` subrole doesn't match the signed-in client user.
  const sections = navSections
    .map(s => ({ ...s, items: s.items.filter(i => !i.only || i.only === user?.client_subrole) }))
