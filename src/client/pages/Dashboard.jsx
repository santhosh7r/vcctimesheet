...
export default function ClientDashboard() {
  ...
  return (
    <>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-6 bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200/60 rounded-2xl p-5">
          ...
          <div className="space-y-2">
            {data.pending_actions.slice(0, 6).map((a, i) => {
              const target = a.kind.startsWith('sow_') ? '/client/sows'
                : a.kind === 'timesheet_approve' ? '/client/timesheets'
                : '/client/invoices';
              return (
      ...
      {/* Billing — last 4 months */}
      {data?.monthly_billing?.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }} className="bg-white border border-slate-200/70 rounded-2xl overflow-hidden mb-6">
          <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <DollarSign size={14} className="text-slate-500" strokeWidth={2} />
              <h3 className="text-[14px] font-semibold text-slate-800">Billing · Last 4 months</h3>
            </div>
            <span className="text-[12px] font-semibold text-slate-700 tabular-nums">
              {fmtMoney(data.monthly_billing.reduce((s, m) => s + (m.total || 0), 0))} total
            </span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-slate-100">
            {data.monthly_billing.map(m => (
              <div key={m.key} className="px-5 py-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{m.label}</p>
                <p className="text-[18px] font-bold text-slate-900 tabular-nums tracking-[-0.02em] mt-1">{fmtMoney(m.total)}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {/* Vendor network — promoted hero block (hidden for client managers) */}
      {user?.client_subrole !== 'manager' && (data?.vendors?.length > 0) && (
        <motion.div
        ...
        >
          ...
          <div className="relative px-6 py-5 flex items-center justify-between border-b border-white/10">
            ...
            <Link to="/client/vendors" className="text-[12px] font-semibold text-white/90 hover:text-white inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-white/15 hover:bg-white/25 backdrop-blur transition-colors">
              Manage vendors →
            </Link>
          </div>

          <div
            className="relative grid gap-px bg-white/10"
            style={{ gridTemplateColumns: `repeat(${Math.min(data.vendors.length, 3)}, minmax(0, 1fr))` }}
          >
            {data.vendors.map(v => (
              <Link
                key={v.id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white border border-slate-200/70 rounded-2xl overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link to="/client/sows" className="text-[11px] font-semibold text-amber-700 hover:text-amber-800">View all →</Link>
          </div>
          <div className="divide-y divide-slate-100">
            {(data?.sows || []).slice(0, 6).map(s => {
              const ss = SOW_STATUS[s.status] || SOW_STATUS.draft;
              return (
                <Link key={s.id} to="/client/sows" className="flex items-center justify-between px-5 py-3 hover:bg-slate-50/50">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white border border-slate-200/70 rounded-2xl overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
          ...
          <Link to="/client/invoices" className="text-[11px] font-semibold text-amber-700 hover:text-amber-800">View all →</Link>
        </div>
