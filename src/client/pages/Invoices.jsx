...
export default function ClientInvoices() {
  ...
  // The client manager has read-only visibility and only sees invoices finance
  // has already approved (not pending ones).
  const visible = useMemo(
    () => isManager ? rows.filter(i => i.client_manager_approved_at) : rows,
    [rows, isManager]
  );

  const totals = useMemo(() => visible.reduce((acc, i) => {
    const a = Number(i.total_amount) || 0;
    acc.total += a;
    if (i.status === 'paid') acc.paid += a;
    else if (i.status !== 'void') acc.outstanding += a;
    return acc;
  }, { total: 0, paid: 0, outstanding: 0 }), [visible]);

  // Finance approves an invoice (records the client-side approval stamp).
  const approveInvoice = async () => {
    setActionLoading(true); setError('');
    try {
      await api.post(`/invoices/${selected.id}/manager-approve`, { user_id: user?.id });
  ...
  return (
    <>
      <PageHeader
        icon={Receipt}
        title="Invoices"
        subtitle={isFinance ? 'Approve invoices and record payments' : isManager ? 'Approved invoices for your account (read-only)' : 'Track all invoices issued to your account'}
      />
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-white border border-slate-200/70 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={Receipt} title={isManager ? 'No approved invoices yet' : 'No invoices yet'} />
        ) : (
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="bg-slate-50/60 border-b border-slate-200/70">
                  ...
                  <th className="text-left font-semibold text-slate-600 px-4 py-3">Approval</th>
                </tr>
              </thead>
              <tbody>
                {visible.map(inv => (
      ...
      <Modal isOpen={!!selected} onClose={() => { setSelected(null); setError(''); }} title={selected ? selected.invoice_number : ''} maxWidth="max-w-2xl">
        {selected && (
          <div className="space-y-4">
            ...
            {/* Finance approves the invoice — the client manager has read-only access. */}
            {isFinance && selected.status !== 'paid' && selected.status !== 'void' && !selected.client_manager_approved_at && (
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                <button onClick={approveInvoice} disabled={actionLoading} className="h-10 px-5 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-[13px] font-semibold shadow-lg shadow-emerald-600/25 hover:from-emerald-600 hover:to-emerald-700 disabled:opacity-60 inline-flex items-center gap-2">
