...
export default function ClientTimesheets() {
  ...
  const approve = useCallback(async (id, e) => {
    e?.stopPropagation();
    setApprovingId(id);
    setError('');
    try {
      await api.post(`/timesheets/${id}/client-approve`);
      setRows(prev => prev.map(t => t.id === id
        ? { ...t, client_approval_status: 'approved', client_approved_at: new Date().toISOString() }
        : t));
    } catch (err) {
  ...
  return (
    <>
      <PageHeader
        ...
        title="Timesheets"
        subtitle={isManager
          ? 'Review and approve submitted timesheets for your team'
          : 'View submitted timesheets from the delivery team for your engagements'}
      />
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="bg-slate-50/60 border-b border-slate-200/70">
                  ...
                  <th className="text-right font-semibold text-slate-600 px-4 py-3">Sign-off</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(t => {
                        ...
                        <td className="px-4 py-3 text-right">
                          {t.client_approval_status === 'approved' ? (
                            <div className="flex flex-col items-end gap-0.5">
                              ...
                              {t.manager_name && <span className="text-[10px] text-slate-400">{t.manager_name}</span>}
                            </div>
                          ) : isManager && t.status === 'submitted' ? (
                            <button
                              onClick={(e) => approve(t.id, e)}
                              disabled={approvingId === t.id}
                              className="inline-flex items-center gap-1 h-7 px-3 rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-[11px] font-semibold shadow-sm shadow-emerald-600/25 hover:from-emerald-600 hover:to-emerald-700 disabled:opacity-60"
                            >
                              <CheckCircle2 size={11} strokeWidth={2.5} /> {approvingId === t.id ? '…' : 'Approve'}
                            </button>
                          ) : (
                            <div className="flex flex-col items-end gap-0.5">
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold bg-amber-50 text-amber-700">
                                <Clock size={10} strokeWidth={2.5} /> Approval Pending
                              </span>
                              {t.manager_name && <span className="text-[10px] text-slate-400">{t.manager_name}</span>}
                            </div>
                          )}
      ...
      <p className="text-[11px] text-slate-400 mt-4 leading-relaxed">
        Timesheets are submitted by the delivery team and added by the VCC admin team.
        {isManager
          ? ' As a client manager you can approve the hours logged against your engagements; your sign-off is recorded separately from the internal approval.'
          : ' This is a read-only view of the hours logged against your engagements.'}
      </p>
    </>
  );
}
