...
import { format, startOfWeek, endOfWeek, addWeeks, isBefore, isAfter, isSameDay, parseISO } from 'date-fns';
...
function getWeeksInPeriod(start, end) {
  const weeks = [];
  // Always start from the Monday of the week containing the period start
...
function getDaysOfWeek(weekStart, weekEnd) {
  // Always show full Mon-Sun week, even if some days fall outside the month
  const days = [];
...
export default function Submissions() {
  ...
  // Get entries for an employee within the fortnightly date range
  const getEmpPeriodData = useCallback((emp) => {
    ...
    // Check date-level coverage: filled = hours > 0 OR has a description (e.g. "leave taken")
  ...
  const filtered = useMemo(() => {
    return employees
      .filter((emp) => !projectFilter || emp.project === projectFilter)
      .map((emp) => {
  ...
  }, [employees, projectFilter, currentPeriod, allTs, tab, getEmpPeriodData]);

  const allMapped = useMemo(() => {
    return employees
      .filter((emp) => !projectFilter || emp.project === projectFilter)
      .map((emp) => getEmpPeriodData(emp));
  }, [employees, projectFilter, currentPeriod, allTs, getEmpPeriodData]);
  ...
  // Auto-open editor when navigating from Freeze page (run once)
  ...
  return (
    <div>
      <PageHeader icon={FileText} title="Employee Submissions" subtitle="Review and edit submitted timesheets" />
              ...
              <table className="w-full">
                ...
                <tbody>
                  {weekDays.map((day) => {
                    ...
                    return (
                      <tr key={dateStr} className={`border-b border-slate-50 ${isOutsideMonth ? 'bg-slate-100/50' : isWeekend ? 'bg-slate-50/60' : 'hover:bg-slate-50/80'} transition-colors`}>
                        <td className="px-2 py-1.5">
                          <div>
                            <p className={`text-[12px] font-medium ${isOutsideMonth ? 'text-slate-300' : isWeekend ? 'text-slate-400' : 'text-slate-700'}`}>
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            ...
                            placeholder={isWeekend ? '—' : 'Work item...'}
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            ...
                            placeholder={isWeekend ? '—' : 'Description...'}
                            className="w-full h-8 px-2 rounded-lg border border-slate-200 bg-white text-[12px] text-slate-700 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-400 transition-all"
                          />
                        </td>
                        <td className="px-2 py-1.5">
